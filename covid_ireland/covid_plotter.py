...
class CovidPlotter(object):
    ...
    def identify_daily_figures(self, counties, roll=0):
        ...
        if roll > 0:
            dailies = self.calculate_rolling_average(7, dailies)

        return dailies

    def calculate_rolling_average(self, roller, data):

        holder = []
        counties = data.groupby('CountyName')
        
        for a, b in counties:
            temp = b.copy()
            temp[self.metric] = temp[self.metric].rolling(roller).mean()
            holder.append(temp)
